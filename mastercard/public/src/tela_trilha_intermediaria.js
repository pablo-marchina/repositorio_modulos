...
class TelaTrilhaIntermediaria extends Phaser.Scene {
    constructor() {
        super({ key: 'telaTrilhaIntermediaria' });
    ...
    preload() {
        this.load.image('fundo_mapa2', 'assets/identidade_de_marca/tela.trilha2.png'); // Imagem de fundo do mapa
    ...
    create() {
        ...
        const fundo = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY, 'fundo_mapa2');
       ...
       this.nivelDesbloqueado = this.registry.get('nivelDesbloqueado_intermediario') || 0;
        ...
        if (btnContinuar) {
            btnContinuar.on('pointerdown', () => {
                fecharPopup(() => {
                        ...
                        const modoSelecionado = this.registry.get('modoDificuldade') || 'iniciante';
                        
                        this.scene.start('contextoIntermediario', {
                            ilha: ilhaEscolhida,
